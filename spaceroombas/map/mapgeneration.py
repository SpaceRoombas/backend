...
def Count_Neighbors(world, x, y, sides):
    ...
    for i in [-1, 0, 1]:
        for j in [-1, 0, 1]:
            if True != (x + i < 0 or x + i >= len(world) or y + j < 0 or y + j >= len(
                    world[0])):  # checks to see if its a live tile and if its out of bound
                if world[x + i][y + j] == 1:
                    count += 1
...
def Update_World(old_world, stay, birth,
                 sides):
    # stay is number of neighbors needed to stay birth is number of neighbors needed to make a  new one
